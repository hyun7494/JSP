...
$(function(){
	...
	$('input[name=pass2]').focusout(function(){
		...
		if(pass1 == pass2){
			if(pass2.match(regPass)){
				isPassOk = true;
				$('.resultPass').css('color', 'green').text('비밀번호 일치');
			}else{
				isPassOk = false;
			...
			isPassOk = false;
			$('.resultPass').css('color', 'red').text('비밀번호 불일치');
		}
	});
	...
	$('input[name=name]').focusout(function(){
		...
		if(!name.match(regName)){
			isNameOk = false;
			$('.resultName').css('color', 'red').text('이름은 한글 2자 이상');
		}else{
			isNameOk = true;
			$('.resultName').text('');
		}
	...
	$('input[name=email]').focusout(function(){
		...
		if(!email.match(regEmail)){
			isEmailOk = false;
			$('.resultEmail').css('color', 'red').text('유효하지 않은 이메일');
		}else{
			isEmailOk = true;
			$('.resultEmail').text('');			
		}
	...
	$('input[name=hp]').focusout(function(){
		...
		if(!hp.match(regHp)){
			isHpOk = false;
			$('.resultHp').css('color', 'red').text('유효하지 않은 휴대폰');
		}else{
			isHpOk = true;
			$('.resultHp').text('');
		}
