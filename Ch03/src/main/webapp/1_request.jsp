...
<html>
	<head>
		...
		<!-- 
			...
			request 내장객체
			 - 클라이언트 요청 정보를 갖는 객체
			 - 클라이언트의 전송 데이터(Parameter)를 수신하는 메서드를 제공
			 
			Get 요청방식
			 - 기본 데이터 전송 방식
			 - 서버에 페이지나 데이터를 요청하는 전송 방식
			 - 전송 데이터가 주소에 노출(? 이후 쿼리 스트링으로; key-value chain 방식으로 출력됨)
			 
			Post 요청방식
			 - 서버에 데이터를 전송하면서 처리를 요청하는 전송 방식
			 - 전송 데이터가 요청 body에 캡슐화되어 전송, 데이터가 주소에 노출되지 x	  
		 -->
	</head>
	<body>
		<h3>request 내장객체</h3>
		...
		<h4>회원가입</h4>
		<form action="./proc/registerProc.jsp" method="post"> <!-- get은 글자 수 제한 있어서 post 전송이 글자 잘릴 위험이 없어 안전 BUT post 전송하면 한글 깨짐 <- post는 데이터 캡슐화해서 보내므로 글자가 인코딩됨 -->
			<table border="1">
				<tr>
					<td>이름</td>
		...
		<h4>Client 정보 확인</h4>
		<p>
			헤더정보: <%= request.getHeader("User-Agent") %><br/> <!-- 사용자 브라우저 종류 -->
